...
async def on_message(message):
    ...
    if not command:
        ...
        if not parameters:
            ...
            await message.channel.send(embed=dus)
            return
        ...
        if word not in data:
            ...
            await message.channel.send(embed=aaem)
            return
...
async def on_ready():
    ...
    print("==========")
    await client.change_presence(status=discord.Status.idle, activity=discord.Game("도움말 및 명령어는 '무군아 도움'"))
